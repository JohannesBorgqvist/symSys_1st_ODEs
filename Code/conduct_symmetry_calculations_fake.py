...
def calculate_symmetries_ODEs(file_name,tangent_degree):
    ...
    if model_reading_successful:
        ...
        for reaction_index in range(len(omega_list)):
            str_reac = omega_list[reaction_index]
            for state_index in range(len(x)):
                str_reac = str_reac.subs(x[state_index],variables[state_index])
        ...
        for tangent_index in range(len(eta_list)):
            str_tangent = eta_list[tangent_index]
            for state_index in range(len(x)):
                str_tangent = str_tangent.subs(x[state_index],variables[state_index])
